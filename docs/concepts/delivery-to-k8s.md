...
продукта. Три операции (публикация, откат, очистка) покрывают полный цикл
эксплуатации релиза и запускаются одинаково в любом окружении.
...
Откат снимает релиз вместе с неймспейсом -- быстро, но грубо: точечного отката
отдельного объекта схема не даёт.
