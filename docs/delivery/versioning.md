...
Плюсы. Единый источник истины -- файл `VERSION` продукта -- через `set-version.sh`
расходится по всем файлам стека (`pom.xml`, `pyproject.toml`, `package.json`) и
дальше в `CI_TAG` и тег образа. Одна правка задаёт версию всему продукту, рассинхрона
между фронтом, беком и образом не возникает.

Минусы. Раскладку версии выполняет shell-скрипт по фиксированному списку файлов:
новый файл с версией нужно добавить в `set-version.sh` вручную, иначе он
останется со старым значением. В демо версия меняется руками, дисциплина bump'а
на стороне инженера.

Безопасность. `CI_TAG` определяет, какой образ уедет в кластер; loose-giterminism
в демо разрывает связь «версия -- содержимое git», поэтому одному тегу может
соответствовать разное рабочее дерево. Боевой режим со строгим giterminism
восстанавливает однозначность -- разбор в
