...
Сборка, деплой и отладка идут на выделенном сервере деплоя (маршрут -- см. infa).
Рабочая машина только пушит код в git; на сервере деплоя выполняется `git pull` и
прогон kube_ci. Все команды ниже выполняются на нём.

## Шаги

### 1. Получить репозиторий

На рабочей машине -- `git push`, на сервере деплоя -- подтянуть актуальное состояние:
