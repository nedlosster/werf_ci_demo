...
Два окружения (dev/prod) и три базовые операции kube_ci ложатся на stage'и
...
В примере публикация и откат разнесены по разным stage'ам одного пайплайна;
на практике откат и очистку удобнее держать отдельными job-ами с ручным запуском
по параметрам `ENV`/`PRODUCT`, чтобы не запускать их при каждой сборке ветки.
...
- В CI продукты не обязательно тянуть через `pull_products.sh` symlink'ами:
  ...
  `00-build-deploy.sh`.
- `01-dissmiss.sh` требует явного product key или `--all` -- значение
  передаётся параметром `PRODUCT`, пустое приведёт к отказу с кодом возврата 1.
  В примере stage `Rollback` поэтому защищён `when`-условием на непустой
  `PRODUCT`.
...
- `WERF_SECRET_KEY` подключается только в stage'ах деплоя; откат и очистка кеша
  ключ шифрования не используют.

## Плюсы, минусы, безопасность

Плюсы. `Jenkinsfile` остаётся обёрткой над теми же тремя скриптами `kube_ci`:
...
только на стадии. Откат завязан на параметр `PRODUCT` и `when`-условие, пустое
значение намеренно валит stage.
