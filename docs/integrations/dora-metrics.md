...
предыдущего (типичная картина hotfix) и запуск `01-dismiss.sh` после деплоя.

Чистого автоматического сигнала «развёртывание сломало систему» в репозитории
нет: и `03-rollback.sh`, и `01-dismiss.sh` запускают как при аварии, так и в
...
- факты запуска скриптов -- какой скрипт (`00-build-deploy.sh`, `03-rollback.sh`,
  `01-dismiss.sh`) и с каким продуктом отработал, видно из лога job.
