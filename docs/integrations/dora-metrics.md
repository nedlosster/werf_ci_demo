...
`01-dissmiss.sh`: откат релиза снимает развёрнутый продукт, и факт его запуска
после деплоя -- это признак неудачной выкатки. Дополнительный сигнал -- повторный
`deploy-prod` с поднятой patch-версией вскоре после предыдущего: типичная
картина hotfix.

Чистого автоматического сигнала «развёртывание сломало систему» в репозитории
нет -- `01-dissmiss.sh` сносит релиз и при плановом выводе продукта, и при аварии,
скрипт эти случаи не различает. Поэтому CFR на контуре собирается с разметкой:
запуски отката и hotfix-выкатки помечаются как инцидент вручную или по
журналу инцидентов, иначе метрика завысит число отказов.
...
работает: повторный `deploy-prod` с исправлением, прошедший без ошибок, или, в крайнем случае,
завершённый `01-dissmiss.sh` плюс возврат предыдущей рабочей версии. Сигналы --
те же метки времени job'ов CI и записи журнала инцидентов; репозиторий даёт
только нижний слой (факты запусков скриптов), верхний слой (когда заметили и
...
- git-история -- коммиты, поднявшие `VERSION`, и их время; связь с выкаткой через
  `CI_TAG`;
- факты запуска скриптов -- какой скрипт (`00-build-deploy.sh`, `01-dissmiss.sh`)
  и с каким продуктом отработал, видно из лога job.
