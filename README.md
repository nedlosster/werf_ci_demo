...
За схемой -- личный опыт разработки и доставки приложений в Kubernetes
2020-2025: не учебный пример, а дистилят практики, сведённый к рабочему минимуму.
...
Окружений намеренно два (dev / prod) -- отдельного `stage` нет: это осознанное
упрощение демо, пары dev/prod достаточно для показа модели. Третий профиль при
необходимости добавляется механически (env-функция в `.helm/def.sh` + каталог
`kube_ci/<env>/`) -- см. [docs/delivery/dev-prod.md](docs/delivery/dev-prod.md).
